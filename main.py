...
keywords = ['img', 'url', 'animate', '-animate', 'def', 'quirk', '-quirk', 'html', 'id']
tagsWithArguments = {'url': {'argc': 1}, 'animate': {'argc': 1}, 'quirk': {'argc': 1}, 'def': {'argc': 1}, 'id': {'argc': 1}}
...
class MSPA_Parser:
    ...
    def loadQuirks(self):
        ...
        if t.is_dir():
            ...
            files = [i for i in quirkFolder.iterdir() if i.is_file() and i.suffix == '.json']
        ...
        if 'default' not in quirks:
            quirks.update({'default': {'color': {'r': 0, 'g': 0, 'b': 0, 'a': 255}}})     
    ...
    def generateCSS(self):
        ...
        for q in self.quirks:
            ...
            if h_bg:
                css.append('.{}[data-highlight="true"] {{\n\tbackground-color: {};\n}}'.format(q, h_bg))
    ...
    def parseTags(self, text):
        ...
        for pos in range(len(text)):
            ...
            if char in ['\n', '\0']:
                raise ParsingError('Tagi nie zostały odpowiednio zamknięte.')
        ...
        if len(t) and len(tags['classes']) != 0:
            raise ParsingError(f'Tag{"" if len(t) == 1 else "i"} "{" ".join(self.__quote(t))}" nie mo{"że" if len(t) == 1 else "gą"} występować z innymi tagami.')
    ...
    def _generageHTML(self, parsedText):
        ...
        for p in parsedText['text']:
            if type(p) == str:
                if len(p):
                    text.append(re.sub('^ ', '', p.replace('<', '&lt;').replace('>', '&gt;')))
            else:
                if 'def' in p['tags']:
        ...
        for i in text:
            if 'url' in parsedText['tags']:
                link = parsedText['tags']['url'][0] if len(parsedText['tags']['url']) else '#'
                text = i if len(i) else link
                args = ' '.join(parsedText['tags']['url'][1:])

                t.append(f'''<a href="{link}"{" " if len(args) else ""}{args}>{text}</a>''')
            elif 'img' in parsedText['tags']:
                args = ' '.join(parsedText['tags']['img'] or [])
                t.append(f'''<img src="{i}"{" " if len(args) else ""}{args}>''')
                ...
                args = ' '.join(args)
                t.append(f'<span class="{" ".join(classes)}"{" " if len(args) else ""}{args}>{preText}{i}</span>')
        return '\n<br>\n'.join(t)
...
<scratch: testtestets>'''
    ...
    with open('out.html', 'w', encoding='utf-8') as f:
        ...
        f.write(f'<p>\n{parser.generateHTML()}\n</p>')
        f.write('<script>document.querySelectorAll(\'[data-highlight]\')[0].dataset.highlight="true"</script>')
